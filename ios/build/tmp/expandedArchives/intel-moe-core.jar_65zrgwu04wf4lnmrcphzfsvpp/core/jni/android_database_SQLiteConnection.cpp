...
static void sqliteCustomFunctionCallback(sqlite3_context *context,
    ...
    if (self == nullptr) {
        printf("a thread is making JNI calls without being attached");
        return;
    }
    JNIEnv* env = (JNIEnv*)self->GetJniEnv();
#else
...
#else
    void* ptr = mmap(NULL, length, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, NULL, 0);
    if (ptr == MAP_FAILED) {
        error = errno;
        ALOGE("mmap failed: %s", strerror(error));
    } else {
