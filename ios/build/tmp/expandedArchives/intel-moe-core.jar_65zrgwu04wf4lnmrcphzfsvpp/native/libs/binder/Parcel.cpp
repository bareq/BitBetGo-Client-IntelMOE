...
status_t Parcel::writeBlob(size_t len, bool mutableCopy, WritableBlob* outBlob)
{
    ...
    void* ptr = ::mmap(NULL, len, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, NULL, 0);
    if (ptr == MAP_FAILED) {
        status = -errno;
    } else {
        status = writeInt32(mutableCopy ? BLOB_ASHMEM_MUTABLE : BLOB_ASHMEM_IMMUTABLE);
        if (!status) {
            status = writeFileDescriptor(NULL, true /*takeOwnership*/);
            if (!status) {
                outBlob->init(NULL, ptr, len, mutableCopy);
                return NO_ERROR;
            }
        }
    }
